...
M=1e8;
Pr_e=0.9; % electric price for V2G ??? Pr_e=sdpvar(XXX) ???
...
P_v2g=sdpvar(length(N_loads),1, 'full'); % V2G active power
Q_v2g=sdpvar(length(N_loads),1, 'full'); % V2G reactive power
...
y_v2g=binvar(length(N_loads),1,'full'); % decision variable denoting whether this V2G station should be invested
%% ***********Constraints*************
Cons=[];
%% 1. Power balance
...
Cons_S=[];
% for i=1:N
%     Cons_S=[Cons_S,In*P_ij==[-(P_load-P_shed-P_dg);g_subs_P],In*Q_ij==[-(Q_load-Q_shed-Q_dg);g_subs_Q]];
% end
Cons_S=[In*P_ij==[-(P_load-P_shed-P_v2g);g_subs_P],In*Q_ij==[-(Q_load-Q_shed-Q_v2g);g_subs_Q], P_shed>=0,Q_shed==P_shed*sqrt(1-n^2)/n];
Cons=[Cons,Cons_S];
...
Cons=[Cons, Cons_PQ];
%% 5. P_ch.*P_dis=0 (P_ch=sdpvar(T,1,'full'),P_dis=sdpvar(T,1,'full')) ??? P_es∈[-P_ch,P_dis] ???
% 0=<P_ch<=z.*P_ch_max,0=<P_dis<=(1-z).*P_dis_max ,z=binvar(T,1,'full')  (Big-M)
%% 6. Energy storage energy balance
% for ti=1~T 
%    E_ti=\sum(t=1~ti) (n_ch*P_ch-P_dis/n_dis)+E_0 
% end
%% **********Objectives*************
Obj_inv=sum(Cost.*y_ij); %investment cost, related to line investment。。。。。。。+y_v2g
Obj_ope=M*sum(P_shed); %+Pr_e*sum(P_dg); Voltage deviation; Line loss
Obj=Obj_inv+Obj_ope;
%% ********* Solve the probelm
