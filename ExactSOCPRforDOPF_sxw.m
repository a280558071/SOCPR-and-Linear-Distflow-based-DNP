...
L=size(netpara,1);%�����е�֧·
r=netpara(:,4)/Zbase;%����,unit: p.u.
x=netpara(:,5)/Zbase;%�翹,unit: p.u.
So=loadpoint(:,3)*1e6/Sbase;%�ڵ㸺�ɹ���,unit: MVA
judge=loadpoint(:,4); %�ڵ�����
I_max=560.98/Ibase; % Ubase=12.35e3, ����ÿ����·��ഫ�� 12MW ���ʣ����Ӧ����ֵI_maxΪ 12e6/Ubase/sqrt(3)= 560.98 A,
v_max=1.1^2;
v_min=0.9^2;
...
In=myincidence(I,J); % �ڵ�֧·��������
Inn=In;
Inn(Inn>0)=0;    % Inn is the negative part of I����ʾ�ӽڵ������֧·�ı��
%% �ڵ���
N=47;
%m=4;
%% ���߱���
...
if result.problem==0
    ...
    display(['��������ܳ�����',num2str(sum(s_Pi([find(judge==0)])),3),' MW']);
    display(['���վ�ܳ�����',num2str(sum(s_Pi(1)),3),' MW']);
    if WithSvolt==1
    ...
    for k=1:L
        ...
        if round(SOC_gap(k),3)==0 % always some errors in duality gap
            %display(['��· ',num2str(I(k)),' ',num2str(J(k)),' SOCP�ɳ�Լ��ȡ�Ⱥţ�'])
        else
